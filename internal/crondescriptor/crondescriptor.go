...
func (cd *CronDescriptor) Parse(expression string) (err error) {
	...
	case splitExprLen == 5:
		log.Printf("Cron expr len is 5 so shift array past seconds element")
		for i, field := range splitExpr {
			parsedExpr[i+1] = field
		}

		log.Printf("%s => %s", splitExpr, parsedExpr)
	...
	case splitExprLen == 6:
		...
		if re.Find([]byte(splitExpr[5])) != nil {
			log.Printf("The last element (%s) appears to have four digits. Assuming a year.", splitExpr[5])
			for i, field := range splitExpr {
				parsedExpr[i+1] = field
			}
		} else {
...
func (cd *CronDescriptor) normalizeExpression(parsedExpr [7]string) ([7]string, error) {
	...
	log.Printf("%v", cd.Options.DayOfWeekIndexZero)
		...
		*/
		...
		if slashFound != -1 && asterixFound == -1 && dashFound == -1 && commaFound == -1 {
			...
			if stepThroughRange != "" {
				...
				log.Printf("Value %s change to %s", stepThroughRange, parsedExpr[i])
			}
		}
	}

	log.Printf("Expression %s normalized to %s", origExpr, parsedExpr)
	return parsedExpr, nil
}

func (cd *CronDescriptor) decreaseDaysOfWeek(weekDay string) (*string, error) {
	log.Printf("Decrease days of week for %s", weekDay)
	var dowChars []string
	...
	for i, c := range weekDay {
		log.Printf("Start at %d for char %s", i, string(c))
		...
		if decrement {
			log.Printf("Decrementing string int %s", string(c))
			...
			log.Printf("Int conversion: %d", charToInt)
			if charToInt == 0 {
	...
	log.Printf("Joining %s into one string", dowChars)
	joinedChars := strings.Join(dowChars, "")
	finalString := &joinedChars

	log.Printf("Final string: %s", *finalString)
...
func (cd *CronDescriptor) getFullDescription() (*string, error) {
	...
	log.Printf("timeSegment: %s", *timeSegment)

	dayOfMonthDesc, err := cd.getDayOfMonthDescription()
	if err != nil {
		return nil, err
	}
	log.Printf("dayOfMonthDesc: %s", *dayOfMonthDesc)

	dayOfWeekDesc, err := cd.getDayOfTheWeekDescription()
	if err != nil {
		return nil, err
	}
	log.Printf("dayOfWeekDesc: %s", *dayOfWeekDesc)

	monthDesc, err := cd.getMonthDescription()
	if err != nil {
		return nil, err
	}
	log.Printf("monthDesc: %s", *monthDesc)

	yearDesc, err := cd.getYearDescription()
	if err != nil {
		return nil, err
	}
	log.Printf("yearDesc: %s", *yearDesc)
...
func (cd *CronDescriptor) getSecondsDescription() (*string, error) {
	...
	log.Printf("getSecondsDescription working with expression %s", expr)
...
func (cd *CronDescriptor) getMinutesDescription() (*string, error) {
	...
	log.Printf("getMinutesDescription working with expression %s", expr)
...
func (cd *CronDescriptor) getHoursDescription() (*string, error) {
	...
	log.Printf("getHoursDescription working with expression %s", expr)
...
func (cd *CronDescriptor) getDayOfTheWeekDescription() (*string, error) {
	...
	log.Printf("getDayOfTheWeekDescription working with expression %s", expr)
	segDesc, err := cd.getSegmentDescription(
		expr,
		DayOfWeek,
		", every day",
		func(s string) (*string, error) {
			expr := s
			log.Printf("Check: \"%s\" contains a #: %t", s, cd.contains(s, []string{"#"}))
		...
		func(s string) string {
			...
			if cd.contains(s, []string{"#"}) {
				...
				if !ok {
					log.Panicf(fmt.Sprintf("No day of the week of the month mapping for: %s, %s", s, dayOfWeekOfMonth))
				}
...
func (cd *CronDescriptor) getMonthDescription() (*string, error) {
	...
	log.Printf("getMonthDescription working with expression %s", expr)
	...
	segDesc, err := cd.getSegmentDescription(
		...
		func(s string) (*string, error) {
			...
			if err != nil {
				...
				if monthInt == -1 {
					log.Panicf("Invalid month value for month: %s", s)
				}
				monthInt++
			}
...
func (cd *CronDescriptor) getDayOfMonthDescription() (*string, error) {
	...
	log.Printf("getDayOfMonthDescription working with expression %s", expr)
...
func (cd *CronDescriptor) getYearDescription() (*string, error) {
	...
	log.Printf("getYearDescription working with expression %s", yearField)
	...
	formatYear := func(y string) (*string, error) {
		...
		if re.Find([]byte(y)) != nil {
			...
			log.Printf("getYearDescription: year converted to time.Date: %d, %s, %d", int(yearInt), strconv.Itoa(year), year)
...
func (cd *CronDescriptor) getSegmentDescription(expr string,
	...
	log.Printf("Current expression: %s", expr)
	...
	case !cd.contains(expr, []string{"/", "-", ","}):
		log.Printf("Expression %s does not contain \"/\", \"-\", \",\"", expr)
		descFormat := getDescFormat(expr)
		log.Printf("Description format: %s", descFormat)
		singleItemDesc, err := getSingleItemDesc(expr)
		if err != nil {
			return nil, err
		}
		log.Printf("Single item format: %s ", *singleItemDesc)
		*description = fmt.Sprintf(descFormat, *singleItemDesc)
		log.Printf("description: %s ", *description)

	case cd.contains(expr, []string{"/"}):
		log.Printf("Expression %s contains \"/\"", expr)

		segments := strings.Split(expr, "/")
		*description = getIntervalDescFormat(segments[1])
		log.Printf("Interval description format: %s", *description)
	...
	case cd.contains(expr, []string{","}):
		log.Printf("Case: \"%s\" contains \",\"", expr)
	...
	case cd.contains(expr, []string{"-"}):
		log.Printf("case contains(expr, []string{\"-\"}):")
...
func (cd *CronDescriptor) generateBetweenSegmentDescription(betweenExpr string,
	...
	log.Printf("Split %s by -", betweenExpr)
	...
	log.Printf("Two segments: %s, %s", *betweenSeg1Desc, *betweenSeg2Desc)
